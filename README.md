...
- **จอ** — Trofeo 9.16 / จอชุดน้ำ 320x320 (auto-detect ตัวที่เสียบอยู่ตอนเปิดแอป)
- **แนว/โหมด** — Trofeo: แนวนอน / เต็มจอ / แนวตั้ง · จอชุดน้ำ: now-playing / viz เต็มจอ
...
## 💧 จอชุดน้ำ 320x320 (`--dev cz`)

รองรับจอ AIO ของ Thermalright อีกตัว (ChiZhu Tech, `87AD:70DB`, จอกลมบนหัวปั๊ม) —
โปรโตคอล **CZ** ส่ง raw RGB565 (ไม่ใช่ JPEG) ดูรายละเอียดใน [`czlcd.py`](czlcd.py)

```bash
python czlcd.py --test               # ภาพทดสอบเช็คทิศ/ขนาดจอ
python send.py pic.png --dev cz --fit cover   # ภาพนิ่งเต็มจอ
python send.py clip.gif --dev cz --loop       # GIF วนเล่น
python vibe.py --dev cz                       # now-playing + spectrum จอเหลี่ยม
python vibe.py --dev cz --full --viz random   # viz เต็มจอ สุ่มสลับสไตล์
python vibe.py --dev cz --lyrics              # เนื้อเพลงคาราโอเกะใต้ปก
```

vibe.py บนจอเหลี่ยมจัด layout ใหม่: **ปกกลางบน (glow เต้นตามบีต) → ชื่อเพลง/ศิลปิน
→ progress → สเปกตรัม 30 แท่งล่าง** — ธีมสีดูดจากปกเหมือนจอใหญ่ทุกอย่าง

Windows ผูก WinUSB ให้อัตโนมัติ (ไม่ต้อง Zadig) · ปิด TRCC ก่อนรันเหมือนเดิม
ถ้าภาพกลับหัวใช้ `--rotate 180`
