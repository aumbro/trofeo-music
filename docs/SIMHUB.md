...
return 'spd='  + Math.round($prop('SpeedKmh'))
     ...
     + ';x='    + $prop('Longitude')               // พิกัดสนาม (track map) — ดูหมายเหตุด้านล่าง
     + ';y='    + $prop('Latitude')
     + ';sec='  + $prop('CurrentSectorIndex')
     + '\n';
```

### 🗺️ พิกัดสนามสำหรับ track map (`x` / `y`)
minimap สร้างจากพิกัดรถ 2 มิติ — `race.py` จดเส้นตอนวิ่ง **1 รอบแรก** แล้วล็อกเป็นรูปสนาม
(ระหว่างนั้นโชว์ "● REC"). **ชื่อ property พิกัดต่างกันมากในแต่ละเกม** เลือกให้ตรงเกม:

| เกม | ใช้เป็น `x` | ใช้เป็น `y` | หมายเหตุ |
|---|---|---|---|
| iRacing | `Longitude` | `Latitude` | GPS lon/lat → ได้รูปสนามเป๊ะ |
| ACC / AC | `GameRawData.Graphics.carCoordinates[0]` | `...carCoordinates[2]` | ใช้ x กับ z (ไม่ใช่ y=ความสูง) |
| Forza | property พิกัด X | property พิกัด Z | หาในตัว picker |

> พิมพ์ในตัว editor แล้วกด autocomplete ค้นคำว่า *coordinate / position / Lat / Lon* — ถ้าเกมไม่มีพิกัดเลย
> map จะไม่ขึ้น (แดชส่วนอื่นทำงานปกติ). `sec` = sector ปัจจุบัน (1/2/3) ใช้ไล่สีเส้นสนาม
...
| `tfl` `tfr` `trl` `trr` | อุณหภูมิยาง °C | int | | `x` / `y` | พิกัดสนาม 2D (track map) | float |
| `sec` | sector ปัจจุบัน 1/2/3 | int | | | | |
